...
export default function WhatsAppSearchScreen({ onNavigate, params }: NavigationProps) {
  const { user, signInWithGoogle, role, loading: authLoading } = useAuth();
  ...
  useEffect(() => {
    const fetchSearchDetails = async () => {
      ...
      try {
        ...
        // 2. Fetch providers for this category
        const { data: providersData, error: providerError } = await supabase
          .from('profiles')
          .select('*')
          .eq('role', 'provider')
          .contains('categories', [search.service_categories?.name]);
    ...
    <div className="min-h-screen bg-black text-white font-display overflow-x-hidden">
      ...
      <main className="relative z-10 max-w-lg mx-auto px-4 py-12 flex flex-col min-h-screen">
        ...
        <div className="text-center mb-10">
          <div className="inline-flex items-center gap-2 bg-emerald-500/10 text-emerald-400 px-4 py-2 rounded-full border border-emerald-500/20 mb-6 animate-fade-in">
          ...
          <h1 className="text-4xl md:text-5xl font-black mb-4 leading-tight">
            Olá! Encontramos <span className="text-primary">{providers.length}</span> especialistas em <span className="text-primary">{categoryName}</span>
          </h1>
        ...
        <div className="flex flex-col gap-4 mb-12">
          {providers.length > 0 ? (
            providers.map((p, idx) => (
              <div 
                key={p.id}
                className={`group relative overflow-hidden bg-white/5 border border-white/10 rounded-3xl p-4 transition-all ${!isLogged && idx > 0 ? 'blur-md opacity-40 select-none' : 'hover:border-primary/50'}`}
              >
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-1.5 mb-1">
                       ...
                       <div className="flex items-center text-yellow-500 gap-0.5 ml-auto">
                          ...
                          <span className="text-xs font-black">{p.rating || '4.9'}</span>
                       </div>
                    </div>
                    <h3 className="font-bold text-lg leading-snug">{p.full_name}</h3>
                    <p className="text-xs text-slate-400 font-medium">✨ {p.bio ? p.bio.substring(0, 40) + '...' : 'Especialista verificado iService'}</p>
                  </div>
                </div>
        ...
        {!isLogged && (
          <div className="mt-auto bg-gradient-to-t from-black via-black/90 to-transparent pt-20 pb-10 sticky bottom-0">
            <div className="bg-primary p-6 rounded-[2rem] shadow-2xl shadow-primary/30 relative overflow-hidden group">
              ...
              <div className="relative z-10 text-center">
                ...
                <p className="text-white/80 text-xs font-bold mb-6">
                  Cadastre-se rapidamente com o Google para ver todos os {providers.length} profissionais e falar com eles agora.
                </p>
