        ...
        <div className="netflix-gutter transition-all duration-300">
          ...
          <div className="mt-3 overflow-x-auto hide-scrollbar flex items-center gap-2 pb-2 transition-all duration-300">
            ...
            <button 
              ...
              className="h-8 flex-shrink-0 flex items-center gap-1.5 px-3 rounded-full border border-white/20 bg-white/5 text-[11px] font-black text-white hover:bg-white/10 transition-all"
            >
              <span className="material-symbols-outlined text-[15px] text-primary">{viewMode === 'list' ? 'map' : 'format_list_bulleted'}</span>
              <span className="leading-none">{viewMode === 'list' ? 'Mapa' : 'Lista'}</span>
            </button>
            ...
            {dynamicCategories.map((cat) => (
              <button
                ...
                className="h-8 flex-shrink-0 px-4 rounded-full text-[11px] font-black transition-all border border-white/20 bg-white/5 hover:border-white hover:bg-white/10 text-gray-300 hover:text-white flex items-center justify-center"
              >
                {cat.name}
              </button>
            ))}
...
function SkeletonRow() {
  return (
    <div className="mb-1.5 md:mb-3 animate-pulse">
        ...
        <div className="flex gap-2">
          {[1, 2, 3, 4, 5].map(i => (
            <div key={i} className="shrink-0 w-[115px] md:w-[200px] lg:w-[280px]">
              <div className="aspect-[2/2.8] md:aspect-video bg-zinc-800 rounded-lg"></div>
            </div>
          ))}
        </div>
      </div>
...
function CollectionRow({ title, subtitle, providers, onNavigate, highlight, onViewMore }: CollectionRowProps) {
  ...
  return (
    <section className="mb-1.5 md:mb-3">
      ...
      <div className="relative group/row">
        ...
        >
          ...
          <div className="flex gap-1.5 md:gap-2">
            ...
            >
              <div className="relative aspect-[2/2.8] md:aspect-video rounded-lg overflow-hidden bg-zinc-900 shadow-lg border border-white/5">
                <img
                  src={p.image}
                  alt={p.name}
                  className="w-full h-full object-cover"
